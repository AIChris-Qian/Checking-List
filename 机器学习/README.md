...
#### 03 基分类器
   ...
   Q：常用的基分类器是什么？
     ...
     决策树 (1) 较为方便地将样本权重整合到训练过程中
           (2) 决策树的表达和泛化能力，通过调节树的层数来折中
           (3) 数据样本的扰动对于决策树影响较大，不稳定的学习器更适合作为基分类器
     
     神经网络也可以作为基分类器，因为神经网络也比较不稳定。
     调整神经元数量 / 连接方式 / 网络层数 / 初始值权值引入随机性。
   
   
   Q：可否将随机森林中的基分类器，由决策树替换成线性分类器或 k-近邻？
   
     Bagging采用的基分类器，对数据分布比较敏感。
     线性分类器或者 k-近邻 都是较为稳定的分类器，本身方差不大。
...
#### 04 偏差和方差
   ...
   Q：什么是偏差和方差？
     ...
     偏差：预测模型输出与真实模型输出的差值
     方差：由训练数据训练出所有模型输出的方差
      
   Q：如何从减小方差和偏差角度解释 Boosting 和 Bagging 的原理？
   
     Bagging：原本的方差为 σ^2，样本平均的方差为 σ^2/n，方差减少。
              重采样带来弱分类器之前的独立性，降低方差。
              
     Boosting：训练好一个弱分类器，计算残差，作为下一个分类器的输入。
               不断减少损失函数，偏差减少。各分类器相关性大，缺乏独立性。
...
#### 05 梯度提升决策树
   ...
   Q：GBDT的基本原理是什么？
     ...
     基于已生成的弱分类器集合的结果，新的弱分类器会重点关注没有被正确分类的样本。
     
     GBDT：根据当前损失函数的信息来训练新加入的弱分类器，将训练好的弱分类器以累加的形式结合到现有模型中。
           GBDT中使用的决策树通常为CART。
...
#### 06 XGBoost 与 GBDT 的联系与区别
   ...
   Q：XGBoost 与 GBDT 的联系和区别有哪些？
     ...
     (2) XGBoost 加入正则化控制模型的复杂度，防止过拟合。
     (3) GBDT 在模型训练时只使用一阶导数信息，XGBoost 可以同时使用一阶或二阶导数信息。
     (4) GBDT 采用 CART 作为基分类器，XGBoost 支持多种分类器。
     (5) GBDT 每轮迭代时使用全部数据，XGBoost 支持对数据进行采样。
     (6) GBDT 不能对缺失值进行处理，XGBoost 能对缺失值进行处理。
