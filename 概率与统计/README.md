...
------------------------------
 ...
 #### 11.1 变量间关系的度量
  ...
  #### 11.2 一元线性回归
   ...
   显著性检验：
     ...
     (2) 回归系数
         ...
         线性关系：自变量与因变量之间的线性关系是否显著
         回归关系：自变量对因变量的影响是否显著
         
         H0：β1 = 0    H1：β1 ≠ 0
         σβ1^ = σ / √￣(∑(xi^ - xi均)^2)    σ 未知，用 se 代替     sβ1^ = se / √￣(∑(xi^ - xi均)^2)
         t = (β1^ - β1) / sβ1^      |t| > t(α/2)，拒绝 H0。
         
         一元线性回归，F 检验和 t 检验是等价的，t 检验拒绝 F 检验也拒绝。
         多元线性回归，两者不同。    F检验：总体回归关系显著性
                                   t检验：各个回归系数显著性
  ...
  #### 11.3 利用回归方程进行预测
   ...
   `y = β0 + β1x + ε`  `ε ~ N(0,σ^2)`  确定`ε`的假定是否成立
   ...
   残差图：
     ...
     横轴自变量 x，纵轴残差 εi = y - y^
     ε 方差相同       ε 方差不同        模型不合理，考虑曲线回归 / 多元回归模型
     
   标准化残差：
   
     zei = εi / se = (y - y^) / se
     ε 服从正态分布，标准化残差也服从正态分布。
...
------------------------------     
   ...
   误差项 ε 三个基本假设：
     ...
     (1) E(ε) = 0   E(y) = β0 + β1x1 + β2x2 + ... + βkxk
     (2) Var(ε) = σ^2
     (3) ε ~ N(0,σ^2)  相互独立   因变量 y 服从正态分布
     
   多元回归方程：`E(y) = β0 + β1x1 + β2x2 + ... + βkxk`
   
   估计多元回归方程：`E(y^) = β0^ + β1^x1 + β2^x2 + ... + βk^xk`
   
   参数的最小二乘法估计：
   
     Q = ∑(yi - yi^)^2 = ∑(yi - yi^)^2 = ∑(yi - β0 - β1x1 - β2x2 - ... - βkxk`)^2 最小
     同时对 β0^ 和 βi^ 求偏导
   ...
   #### 12.2 回归方程的拟合程度
     ...
     SST = SSR + SSE
     SST = ∑(yi - yi均)^2      SSR = ∑(yi^ - yi均)^2      SSE =  ∑(yi - yi^)^2
             总平方和                   回归平方和                 残差平方和
        
     R^2 = SSR / SST = 1 - SSE / SST (多重制定系数 / 负相关系数)
                                      增加自变量，SSE减小，SSR变大，R^2变大
     为了避免增加自变量而高估 R^2    R^2(a) = 1 - (1 - R^2)((n - 1) / (n - k - 1))    (调整的多重制定系数)    
                                   n：样本量     k：自变量个数
     估计标准误差：  se = √￣(SSE / (n-k-1)) = √￣(MSE) 
                                      
         
   #### 12.3 显著性检验       
   
   多元回归中，线性关系 (F检验) 和回归系数 (t检验) 不等价
    
   线性关系：
       
     有一个自变量与因变量的线性关系显著，F检验成立
   
   回归系数：
   
     每一个自变量与因变量的关系显著，t检验成立
        
   线性关系检验：总体显著性检验
   
     (1) 提出假设
         H0：β1 = β2 = ..... = βk = 0 
         H1：β1 = β2 = ..... = βk 至少一个不等于 0      
     (2) 计算检验统计量 F
         F = (SSR / k) / (SSE / (n-k-1)) = MSR / MSE ~ F(k,n-k-1)
     (3) 决策    
         F > Fa，拒绝原假设
   ...
   判别：
     ...
     (1) F 检验显著时，βi 的 t 检验不显著
     (2) 回归系数的正负号与预期相反
      
   处理：将相关的自变量从模型中剔除，保留的自变量不相关。
   ...
   一个以上变量引入回归模型时，能否使 SSE 减小。  F 统计量
...
------------------------------     
   ...
   时间序列：    
     ...
     Yi(i=1,2,3,....,n)    ti = 时间    Yi = 观测值
     平稳序列 + 非平稳序列
     平稳序列：波动在固定水平上没有规律，是随机的。    没有趋势
     非平稳序列：包括趋势 / 季节性 / 周期性的序列     有趋势 / 有趋势 + 季节性 / 混合
                趋势：线性或非线性，持续上升或下降     (T)
                季节性：一年内重复出现的周期性的波动   (S)
                周期性：周期一年以上，周期长短不一     (C)
                随机性：除此以外的偶然性波动          (I)
     乘法模型：Yt = Tt * St * Ct * It
   ...
   增长率分析：
     ...
     (1) 增长率  环比增长率   Gi = (Yi - Yi-1) / Yi-1 = Yi / Yi-1 - 1
                定基增长率   Gi = (Yi - Y0) / Y0 = Yi / Y0 - 1
     (2) 平均增长率          G均 = n^√￣(Yn / Y0) -1
     (3) 增长 1% 的绝对值    前期水平 / 100
   ...
   选择预测方法：
     ...
     平滑法预测：没有趋势    简单平均法 / 移动平均法 / 指数平滑法
     季节性预测：有季节性    多元回归 / 自回归 / 时间序列分解
     趋势预测：  有趋势      线性趋势 / 非线性趋势 / 自回归
   ...
   非线性趋势：
     ...
     (1) 指数曲线
         Yi^ = bo*b1^t   log(Yt^) = log(bo) + t*log(b1)
         最小二乘法求解 log(bo) 和 log(b1)  
          
     (2) 多阶曲线
         Yt^ = bo + b1*t + b2*t^2 + ... + bk*t^k
         多元线性回归 (最小二乘法)
          
          
   #### 13.6 复合型序列的分解预测 
   
   含有趋势 / 季节 / 周期 / 随机成分的序列   Yt = Tt * St * It
  ...
  确定并分离季节成分：
     ...
     (1) 计算季节指数
         计算移动平均值，并进行中心化处理，得到中心化移动平均值 (CMA)
         计算移动平均值比值   观察值 / 中心化移动平均值
         季节指数调整     每个季节指数平均值 / 总平均值
     (2) 分离季节成分
         Y / S = T * S * I / S = T * I
